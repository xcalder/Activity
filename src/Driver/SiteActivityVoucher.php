 ...
 *
 */
class SiteActivityVoucher implements ActivityInterface
{
    /**
     * 添加规则
     */
    ...
    public static function addActivityProduct($request){
        ...
        foreach ($rules_products as $key=>$value){
            if(isset($value['product_id'])){
                ...
                $products[$i]['activity_rules_id'] = 0;
                $products[$i]['status'] = 3;
                $i++;
            }
        }
            ...
            <div class="modal-content p-3" style="min-height: 600px">
                ...
                <!-- Nav tabs -->
                  <ul class="nav nav-tabs" role="tablist">
                    <li role="presentation" class="active"><a data-status="3" href="#passed" aria-controls="passed" role="tab" data-toggle="tab" >已通过</a></li>
    ...
    public static function getApplyForm($request){
                  ...
                  <div class="tab-content">
                    <div role="tabpanel" class="tab-pane active" id="passed">
                        <form id="unjoined-form" action="$action_del_product_to_rule" method="post" enctype="multipart/form-data">
                            <input type="hidden" name="api_token" value="$api_token">
                            <input type="hidden" name="id" value="$id">
                            <table class="table"><thead><tr><td><button class="btn btn-default btn-sm" type="submit">移出</button></td></tr><tr><td><input type="checkbox" class="select-all">全选</td><td>规格</td><td>销量</td><td>商品名</td></tr></thead><tbody></tbody><tfoot></tfoot></table>
                        </form>
                    </div>
                    <div role="tabpanel" class="tab-pane" id="not-joined">
                        <form id="add-rule-products" method="post" enctype="multipart/form-data" action="$action_add_product_to_rule">
        ...
        <script type="text/javascript">
            ...
            function getActivityRulesProducts(){
                ...
                $.ajax({
            	    type: 'GET',
            	    url: url+'/api/activity/get_activity_rule_products?status='+data_status+'&site_role=$site_role',
            	    data: {api_token: api_token,id:$id},
            	    dataType: 'json',
            	    success: function(data){
            	    	if(data.status){
                            ...
                            for(var i in products){
                                ...
                                for(var d in product.roles_price){
                                    ...
                                    html += '</td><td>'+product.sales_volume+'</td>';
                                    
                                    html += '<td><img src="'+product.thumb_img+'">'+product.title+'</td></tr>';
                                }
                                e++;
                            }
